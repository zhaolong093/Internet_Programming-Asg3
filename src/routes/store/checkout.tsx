...
function CheckoutPage() {
  ...
  function validatePayment() {
    ...
    if (payment.cardNumber.replace(/\s/g, "").length !== 16)
      e.cardNumber = "Enter a valid 16-digit number.";
        ...
        <div className="flex items-center gap-2">
          {(["address", "payment"] as Step[]).map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              <div
                className={`flex h-8 w-8 items-center justify-center rounded-full text-sm font-bold transition-colors ${step === s ? "bg-primary text-primary-foreground" : i < (step === "payment" ? 1 : 0) ? "bg-success text-success-foreground" : "bg-muted text-muted-foreground"}`}
              >
                {i < (step === "payment" ? 1 : 0) ? <CheckCircle2 className="h-4 w-4" /> : i + 1}
              </div>
              <span
                className={`text-sm font-medium capitalize ${step === s ? "text-foreground" : "text-muted-foreground"}`}
              >
                {s}
              </span>
            ...
            <>
              ...
              <div className="space-y-4">
                <Field
                  ...
                  onChange={(v) => setPay((p) => ({ ...p, cardName: v }))}
                  placeholder="Jane Smith"
                />
                <div className="space-y-1.5">
                  ...
                  {payErrors.cardNumber && (
                    <p className="text-xs text-destructive">{payErrors.cardNumber}</p>
                  )}
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        setPay((p) => ({
                          ...p,
                          expiry: c.length >= 3 ? c.slice(0, 2) + "/" + c.slice(2) : c,
                        }));
                      }}
                    />
                    {payErrors.expiry && (
                      <p className="text-xs text-destructive">{payErrors.expiry}</p>
                    )}
                  </div>
                  <div className="space-y-1.5">
