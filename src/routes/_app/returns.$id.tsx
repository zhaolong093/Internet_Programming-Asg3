...
function ReturnDetail() {
  ...
  const total = Math.max(
    0,
    r.refundAmount - (r.refundAmount * restock) / 100 - (shipDeduct ? 9.99 : 0),
  );
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3 no-print">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <Link to="/returns" className="inline-flex items-center gap-1 hover:text-foreground">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_360px]">
        <div className="space-y-6">
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Submitted {format(new Date(r.submittedAt), "PPpp")} · Updated{" "}
                  {format(new Date(r.updatedAt), "PP")}
                </p>
              </div>
            </div>
            <ol className="mt-6 flex items-center gap-2">
              {STEPS.map((s, i) => (
                <li key={s} className="flex flex-1 items-center gap-2">
                  <div
                    className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-semibold ${i <= stepIdx ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
                  >
                    {i + 1}
                  </div>
                  <div
                    className={`text-xs ${i <= stepIdx ? "text-foreground" : "text-muted-foreground"}`}
                  >
                    {s}
                  </div>
                  {i < STEPS.length - 1 && (
                    <div className={`h-0.5 flex-1 ${i < stepIdx ? "bg-primary" : "bg-muted"}`} />
                  )}
                </li>
              ))}
            </ol>
            ...
            <div className="mt-4 flex gap-4">
              ...
              <div className="text-sm">
                ...
                <div className="text-muted-foreground">
                  SKU {r.product.sku} · {r.product.variant}
                </div>
                <div className="mt-2 grid grid-cols-2 gap-x-6 gap-y-1 text-xs text-muted-foreground">
                  <div>
                    Qty: <span className="text-foreground">{r.qty}</span>
                  </div>
                  <div>
                    Order: <span className="text-foreground">{r.orderId}</span>
                  </div>
                  <div>
                    Purchased:{" "}
                    <span className="text-foreground">{format(new Date(r.orderDate), "PP")}</span>
                  </div>
                  <div>
                    Unit price:{" "}
          ...
          <section className="rounded-xl border bg-card p-6 shadow-sm">
            ...
            <blockquote className="mt-3 border-l-4 border-primary/40 bg-muted/40 p-3 text-sm italic text-muted-foreground">
              "{r.explanation}"
            </blockquote>
            <div className="mt-4">
              ...
              <div className="flex gap-3 overflow-x-auto">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="flex h-24 w-32 shrink-0 flex-col items-center justify-center rounded-md border bg-muted/40 text-xs text-muted-foreground"
                  >
          ...
          <section className="rounded-xl border bg-card p-6 shadow-sm">
            ...
            <ul className="mt-4 space-y-3 border-l pl-4">
              <li className="relative text-sm">
                <span className="absolute -left-[18px] top-1.5 h-2.5 w-2.5 rounded-full bg-info" />
                Return submitted by {r.customer.name} · {format(new Date(r.submittedAt), "PP")}
              </li>
              {r.notes.map((n) => (
                <li key={n.id} className="relative text-sm">
                  <span className="absolute -left-[18px] top-1.5 h-2.5 w-2.5 rounded-full bg-primary" />
                  <div className="flex items-center gap-2">
                    <Avatar size="sm" name={n.author} />
        ...
        <aside className="space-y-6">
          <section className="rounded-xl border bg-card p-5 shadow-sm">
            ...
            <div className="mt-3 grid grid-cols-1 gap-1 text-sm">
              ...
              <div className="text-muted-foreground">
                Returns: <span className="text-foreground">{customer.totalReturns} previous</span>
              </div>
              <div className="text-muted-foreground">
                Customer since {format(new Date(customer.customerSince), "PP")}
              </div>
            </div>
          </section>
          ...
          <section className="rounded-xl border bg-card p-5 shadow-sm">
            ...
            <div className="mt-3 grid grid-cols-1 gap-1 text-sm">
              <div className="text-muted-foreground">
                Order: <span className="text-foreground font-mono">{r.orderId}</span>
              </div>
              <div className="text-muted-foreground">
                Date: <span className="text-foreground">{format(new Date(r.orderDate), "PP")}</span>
              </div>
              <div className="text-muted-foreground">
                Address: <span className="text-foreground">{r.shippingAddress}</span>
              </div>
              <div className="text-muted-foreground">
                Payment: <span className="text-foreground">{r.paymentMethod}</span>
              </div>
            </div>
          </section>
          ...
          <section className="rounded-xl border bg-card p-5 shadow-sm">
            ...
            <dl className="mt-3 space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <dd>${r.refundAmount.toFixed(2)}</dd>
              </div>
              <div className="flex items-center justify-between">
              ...
              <div className="flex items-center justify-between">
                <dt className="text-muted-foreground">Shipping deduction</dt>
                <dd>
                  <Checkbox checked={shipDeduct} onCheckedChange={(v) => setShipDeduct(!!v)} />
                </dd>
              </div>
              <div className="flex justify-between border-t pt-3 font-display text-lg font-semibold text-success">
                <dt>Total Refund</dt>
