...
function ReportsPage() {
  ...
  function downloadCsv() {
    ...
    a.href = url; a.download = "lreturns-report.csv"; a.click();
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={<div className="flex flex-wrap items-center gap-1.5">{PRESETS.map((p) => (<button key={p} onClick={() => setPreset(p)} className={cn("rounded-full px-3 py-1 text-xs font-medium", preset === p ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-accent")}>{p}</button>))}</div>}
      />
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="rounded-xl border bg-card p-6 shadow-sm">
          ...
          <div className="mt-4"><BarChart data={monthlyVolume.map((m) => ({ label: m.m, value: m.v }))} /></div>
        </div>
        <div className="rounded-xl border bg-card p-6 shadow-sm">
          ...
          <div className="mt-4"><BarChart data={monthlyRefunds.map((m) => ({ label: m.m, value: m.v }))} valueFormat={(n) => `$${n}`} color="var(--success)" /></div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        <div className="rounded-xl border bg-card p-6 shadow-sm">
          ...
          <div className="mt-4"><BarChart data={reasonsBreakdown.map((r) => ({ label: r.label.split(" ")[0], value: r.value }))} /></div>
        </div>
        <div className="rounded-xl border bg-card p-6 shadow-sm">
      ...
      <div className="rounded-xl border bg-card shadow-sm">
        <div className="flex items-center justify-between p-5">
          <div><h3 className="font-display text-base font-semibold">Detailed Breakdown</h3><p className="text-xs text-muted-foreground">Monthly summary</p></div>
          <Button variant="outline" onClick={downloadCsv}><Download className="mr-1 h-4 w-4" />Download Report</Button>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-muted/40 text-[11px] uppercase tracking-wide text-muted-foreground">
              <tr><th className="px-4 py-2 text-left">Month</th><th className="px-4 py-2 text-right">Total</th><th className="px-4 py-2 text-right">Approved</th><th className="px-4 py-2 text-right">Rejected</th><th className="px-4 py-2 text-right">Avg Days</th><th className="px-4 py-2 text-right">Refund Total</th><th className="px-4 py-2 text-right">Recovery %</th></tr>
            </thead>
            <tbody>
