    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          extra={
            <div className="mt-3 h-1 w-full overflow-hidden rounded-full bg-muted">
              <div
                ...
                style={{ width: `${totalReturns > 0 ? (pendingCount / totalReturns) * 100 : 0}%` }}
              />
            </div>
          }
        />
        <KpiCard
          ...
          trend={{
            value: `${totalReturns > 0 ? Math.round((approvedCount / totalReturns) * 100) : 0}% approval rate`,
            up: true,
          }}
          accent="success"
          icon={CheckCircle2}
        ...
        <KpiCard
          ...
          value={`$${revenueRecovered.toLocaleString(undefined, { maximumFractionDigits: 0 })}`}
          subtitle="From refunded returns"
          accent="info"
          icon={DollarSign}
          loading={loading}
        />
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_360px]">
        <div className="space-y-6">
          <div className="rounded-xl border bg-card p-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 items-center gap-6 md:grid-cols-[200px_1fr]">
              <DonutChart
                data={reasonsBreakdown.map((r) => ({ label: r.label, value: r.value }))}
              />
                  ...
                  return (
                    <li key={r.label} className="flex items-center justify-between gap-3">
                      ...
                      <span className="tabular-nums text-muted-foreground">
                        {r.value} <span className="text-xs">· {pct}%</span>
                      </span>
                    </li>
                  );
                })}
          ...
          <div className="rounded-xl border bg-card shadow-sm">
            <div className="flex flex-col gap-3 p-5 md:flex-row md:items-center md:justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <Link to="/returns" className="text-sm font-medium text-primary hover:underline">
                  View All
                </Link>
              </div>
            </div>
        ...
        <aside className="space-y-6">
          <Button
            className="w-full"
            onClick={() => toast.success("Manual return draft created (mock).")}
          >
          ...
          <div className="rounded-xl border bg-card p-5 shadow-sm">
            ...
            <p className="text-xs text-muted-foreground">
              {pendingCount} return{pendingCount !== 1 ? "s" : ""} awaiting action
            </p>
                  ...
                  <li key={p.id} className="flex flex-col gap-2 rounded-lg border p-3 text-sm">
                    <div className="min-w-0">
                      ...
                      <div className="truncate text-xs text-muted-foreground">
                        {p.product.name} · ${p.refundAmount.toFixed(2)}
                      </div>
                    </div>
                    <div className="flex gap-2">
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        className="h-7 flex-1 border-destructive/30 text-destructive hover:bg-destructive/10"
          ...
          <div className="rounded-xl border bg-card p-5 shadow-sm">
            ...
            <p className="text-xs text-muted-foreground">
              {pendingOrders.length} order{pendingOrders.length !== 1 ? "s" : ""} to process
            </p>
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">
                          {o.items.reduce((s, i) => s + i.qty, 0)} items · ${o.total.toFixed(2)}
                        </div>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <Button
                        ...
                        className="h-7 flex-1 bg-success text-success-foreground hover:bg-success/90"
                        onClick={() => {
                          useOrderStore.getState().updateStatus(o.id, "processing");
                          toast.success(`${o.id} approved`);
                        }}
                      >
                        Approve
                      </Button>
                      <Button
                        ...
                        className="h-7 flex-1 border-destructive/30 text-destructive hover:bg-destructive/10"
                        onClick={() => {
                          useOrderStore.getState().updateStatus(o.id, "cancelled");
                          toast.message(`${o.id} rejected`);
                        }}
                      >
                        Reject
                      </Button>
            ...
            <ul className="mt-3 space-y-3">
              {activity.map((a) => (
                <li key={a.id} className="relative pl-5 text-sm">
                  <span
                    className={`absolute left-0 top-1.5 h-2 w-2 rounded-full ${a.type === "approved" ? "bg-success" : a.type === "rejected" ? "bg-destructive" : a.type === "refunded" ? "bg-primary" : a.type === "submitted" ? "bg-info" : "bg-muted-foreground"}`}
                  />
                  <div>{a.message}</div>
                  <div className="text-xs text-muted-foreground">
